                    ...
                    <div class="adm-toolbar">
                        <div class="search" aria-label="Search">
                            ...
                            <input id="q" type="text"
                                placeholder="Search name / address / clearance / reason" />
                        </div>
    ...
    <div id="emailLoadingModal" class="modal-overlay" hidden>
        <div class="modal" style="max-width:340px;">
            <div class="modal-body" style="text-align:center;padding:28px 24px 30px;">
                <img src="{{ asset('img/logo_zed.png') }}" alt="Loading logo"
                    style="width:72px;height:72px;object-fit:contain;display:block;margin:0 auto 14px;animation:pulse 1.1s ease-in-out infinite;" />
    ...
    <div id="requestImageModal" class="modal-overlay" hidden>
        <div class="modal" style="max-width:760px;">
            ...
            <div class="modal-body" style="display:flex;flex-direction:column;gap:1rem;align-items:center;">
                <img id="requestImagePreview" alt="Uploaded ID"
                    style="max-width:100%;max-height:78vh;border-radius:14px;border:1px solid #e5e7eb;object-fit:contain;background:#fff;" />
    ...
    <script>
        ...
        function getRecipientEmail(req) {
            ...
            for (const p of profiles) {
                ...
                const profileOwner = String(p.user_key || p.userKey || p.id || p.username || p.fullname || p.name || '')
                    .trim().toLowerCase();
                const profileName = String(p.fullname || p.name || p.username || '').trim().toLowerCase();

                if ((reqOwner && profileOwner && reqOwner === profileOwner) || (reqName && profileName && reqName ===
                        profileName)) {
                    return profileEmail;
                }
            }
            ...
            const html = items
                ...
                .map((r) => {
                    ...
                    return '<div style="padding:.65rem .7rem;border-radius:10px;border:1px solid #eef2f7;background:#fff;margin:.35rem;display:flex;justify-content:space-between;align-items:center">' +
                        ...
                        '<div style="font-size:.82rem;color:#374151;margin-bottom:.4rem;">' + (status ? status.charAt(0)
                            .toUpperCase() + status.slice(1) : 'Pending') + '</div>' +
                        '<div><button type="button" class="btn-mini open-request" data-ref="' + ref +
                        '" style="font-size:.75rem;padding:.25rem .5rem;border-radius:6px">Open</button></div>' +
                        '</div>' +
                        '</div>';
                })
                .join('');
        ...
        async function sendRequestByEmail(ref, overrideEmail) {
            ...
            const payload = {
                recipient,
                subject: 'Certificate Ready for Claim - ' + String(req.ref || '').trim(),
                message: 'Hello ' + String(req.name || 'Resident').trim() + '\n\n' +
                    'Your certificate is now ready.\n' +
                    'Please claim it at the barangay office.\n\n' +
                    'Request details:\n' +
        ...
        function renderTable(requests) {
            ...
            const filtered = requests.filter(r => {
                ...
                const matchQ = !q || name.includes(q) || email.includes(q) || address.includes(q) || age.includes(
                    q) || purpose.includes(q) || reason.includes(q);
            ...
            rows.innerHTML = filtered.map((r, idx) => {
                ...
                const actions = st === 'pending' ?
                    ...
                    '<button class="btn-mini approve" data-action="approve" data-ref="' + ref +
                    '">Approve</button> ' +
                    ...
                    '<button class="btn-mini view" data-action="view" data-ref="' + ref + '">View</button> ' +
                    '<button class="btn-mini edit" data-action="edit" data-ref="' + ref + '">Edit</button> ' +
                    '<button class="btn-mini email" data-action="send-email" data-ref="' + ref +
                    '">Send Email</button> ' +
                    '<button class="btn-mini delete" data-action="delete" data-ref="' + ref + '">Delete</button>';
        ...
        function normalizeServerRequest(request) {
            ...
            return {
                ...
                savedCertType: String(request.savedCertType || request.saved_cert_type || '').trim(),
                savedPaperHtml: String(request.savedPaperHtml || request.saved_paper_html || '').trim(),
            };
        }
